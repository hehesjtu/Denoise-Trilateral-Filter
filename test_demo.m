...
% write the video
video_denoise = VideoWriter('denoise_far.avi','Motion JPEG AVI');
...
for num = 3: numFrames-2
    ...
    for i = pad_pixel+1:blocksize:pad_pixel+height
        for j = pad_pixel+1:blocksize:pad_pixel+width
            ...
            % match error in spatial domain
            wl2 = MatchError(block,blockl2); wl1 = MatchError(block,blockl1);
            wr2 = MatchError(block,blockr2); wr1 = MatchError(block,blockr1);
            % match error in gradient domain 
            gwl2 = MatchError(gblock,gblockl2); gwl1 = MatchError(gblock,gblockl1);
            gwr2 = MatchError(gblock,gblockr2); gwr1 = MatchError(gblock,gblockr1);
            % match error in pixel domain
            wpl2 = (abs(mvl2(mvi,mvj,1,num-2))+abs(mvl2(mvi,mvj,2,num-2)));
            wpl1 = (abs(mvl1(mvi,mvj,1,num-2))+abs(mvl1(mvi,mvj,2,num-2)));
            wpr2 = (abs(mvr2(mvi,mvj,1,num-2))+abs(mvr2(mvi,mvj,2,num-2))); 
            wpr1 = (abs(mvr1(mvi,mvj,1,num-2))+abs(mvr1(mvi,mvj,2,num-2)));
            
            % update the weights 
            sumw = wl2*gwl2*wpl2+wl1*gwl1*wpl1+wr2*gwr2*wpr2+wr1*gwr1*wpr1;
            w1 = wl2*gwl2*wpl2/sumw;  w2 = wl1*gwl1*wpl1/sumw;
            w3 = wr2*gwr2*wpr2/sumw;  w4 = wr1*gwr1*wpr1/sumw; 

            % update the current block
            block = w1*blockl2+ w2*blockl1 + w3*blockr2+ w4*blockr1;
            block_gt = GT((mvi-1)*blocksize+1:mvi*blocksize,(mvj-1)*blocksize+1:mvj*blocksize,:);
            block_cmp = CMP((mvi-1)*blocksize+1:mvi*blocksize,(mvj-1)*blocksize+1:mvj*blocksize,:);
            % L1 loss
            Loss1 = 1./MatchError(block,block_gt);
            Loss2 = 1./MatchError(block_cmp,block_gt);
            if Loss2 < Loss1
                imgprepad(i:i+blocksize-1,j:j+blocksize-1,:)=uint8(block_cmp);
            else
                imgprepad(i:i+blocksize-1,j:j+blocksize-1,:)=block;
            end
        end
    end
